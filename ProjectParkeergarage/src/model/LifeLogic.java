...
public class LifeLogic extends AbstractModel implements Runnable {
	 ...
	 * @throws SimException
	 */
	public void setSize(int size) throws SimException {
		if (size<MIN_SIZE) {
			throw new SimException("Size too small");
		}
		if (size>MAX_SIZE) {
			throw new SimException("Size too large");
		}
		this.size=size;
		sizeIsSet=true;
	 ...
	 * @throws SimException
	 */
	public void setDegree(float degree) throws SimException {
		if (degree<MIN_DEGREE) 
			throw new SimException("Degree too small");
		if (degree>MAX_DEGREE)
			throw new SimException("Degree too large");
		this.degree=degree;
		degreeIsSet=true;
		initRun=false;
	}
	 ...
	 * @throws SimException
	 */
	public void doStep() throws SimException {
		if (!sizeIsSet || !degreeIsSet)
			throw new SimException("Size and/or degree is not set yet");
		if (!initRun)
			throw new SimException("Run init first");
		calculateRound();
		notifyViews();
	}
	 ...
	 * @throws SimException
	 */
	public void doSteps(int numOfSteps) throws SimException {
		if (!sizeIsSet || !degreeIsSet)
			throw new SimException("Size and/or degree is not set yet");
		if (!initRun)
			throw new SimException("Run init first");
	 ...
	 * @throws SimException 
	 */
	public void randomInit() throws SimException {
		if (!sizeIsSet || !degreeIsSet)
			throw new SimException("Size and/or degree is not set yet");
